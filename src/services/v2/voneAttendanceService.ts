...
export class VoneAttendanceService {
  ...
  async closeAttendanceMonth(params: V2BodyParams): Promise<FoxApiResult> {
    const { token, siteId, query, body, cancelId } = params;
  ...
  async reopenAttendanceMonth(params: V2BodyParams): Promise<FoxApiResult> {
    const { token, siteId, query, body, cancelId } = params;
    return this.apiClient.post(v2Path(siteId, "/attendance-reopen", query), { header: authHeader(token), body, cancelId });
  }

  /** GET /company-holidays */
  async listCompanyHolidays(params: V2BaseParams): Promise<FoxApiResult> {
    const { token, siteId, query, cancelId } = params;
  ...
  async deleteCompanyHoliday(params: V2BodyParams): Promise<FoxApiResult> {
    const { token, siteId, query, body, cancelId } = params;
