...
export class VoneMessagesService {
  ...
  async patchVoneMessageTemplate(params: V2BodyParams): Promise<FoxApiResult> {
    const { token, siteId, query, body, cancelId } = params;
    return this.apiClient.patch(v2Path(siteId, "/message-template", query), { header: authHeader(token), body, cancelId });
  }

  /** DELETE /message-template */
  async deleteVoneMessageTemplate(params: V2BodyParams): Promise<FoxApiResult> {
    const { token, siteId, query, body, cancelId } = params;
