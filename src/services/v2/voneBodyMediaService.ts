...
export class VoneBodyMediaService {
  ...
  /** POST /vone-image */
  async uploadVoneBodyMedia(params: V2BodyParams): Promise<FoxApiResult> {
  ...
  async getVoneMediaUploadPolicy(params: V2BaseParams): Promise<FoxApiResult> {
  ...
  async issueVoneMediaUploadURLs(params: V2BodyParams): Promise<FoxApiResult> {
    const { token, siteId, query, body, cancelId } = params;
  ...
  async cancelVoneMediaUpload(params: V2BodyParams): Promise<FoxApiResult> {
    const { token, siteId, query, body, cancelId } = params;
  ...
  async commitVoneMediaUpload(params: V2BodyParams): Promise<FoxApiResult> {
    const { token, siteId, query, body, cancelId } = params;
    return this.apiClient.post(v2Path(siteId, "/vone-media-commit", query), { header: authHeader(token), body, cancelId });
  }

  /** GET /vone-image */
  async downloadVoneBodyMedia(params: V2BaseParams): Promise<FoxApiResult> {
    const { token, siteId, query, cancelId } = params;
