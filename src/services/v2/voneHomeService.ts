...
export class VoneHomeService {
  ...
  async undoReadAllVoneNotifications(params: V2BodyParams): Promise<FoxApiResult> {
    const { token, siteId, query, body, cancelId } = params;
  ...
  async readVoneNotificationsByTarget(params: V2BodyParams): Promise<FoxApiResult> {
    const { token, siteId, query, body, cancelId } = params;
    return this.apiClient.post(v2Path(siteId, "/notification-target-read", query), { header: authHeader(token), body, cancelId });
  }

  /** GET /notification-settings */
  async listVoneNotificationSettings(params: V2BaseParams): Promise<FoxApiResult> {
    const { token, siteId, query, cancelId } = params;
